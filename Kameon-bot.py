...
db = sqlite3.connect(os.path.join(DIR, "ranked.db"))
SQL = db.cursor()
...
class Rank(commands.Cog):
    def __init__(self, Bot):
        self.Bot = Bot
        self.users = {}
        self.guild = 701723124202471455  # id вашей гильдии
        self.channel = 717819890404163694  # id текстового канала, куда будут присылаться сообщения
...
async def on_ready():
    ...
    for mem in guild.members:
            SQL.execute(f'SELECT id FROM users')  # ids
            ids = SQL.fetchall()

            if mem.id not in [a[0] for a in ids]:
                if not mem.bot:
                    SQL.execute(
                        f"INSERT INTO users (id, xp, lvl, msg, name) VALUES ({mem.id}, 50, 1, 1, '{mem.name}')")
                    db.commit()
                    print(f'Для пользователя {mem.name} было добавлено поле')
        print('Все ок [Ranked]')
        db.commit()
...
async def on_message(msg):
    if msg.author == Bot.user:
        ...
        SQL.execute(f"SELECT * FROM users WHERE id={msg.author.id}")
        res = SQL.fetchall()
        if not res:
            SQL.execute(f"INSERT INTO users (id, xp, lvl, msg, name) VALUES ({msg.author.id}, 50, 1, 1, '{msg.author.name}')")
            db.commit()
        else:
            res = res[0]
            lvl = res[2]
            ...
            msgR = res[3]
            msgR += 1
            c = self.Bot.get_channel(channel)  # id текстового канала, куда будут присылаться сообщения
            xp += 25
            if xp == 1000 + 100 * lvl:
                xp = 0
                lvl += 1
                try:
                    ...
                    await c.send(
                        f"{msg.author.mention} Так как у тебя выключен лс, напишу тут, поздравляю! Ты достиг **{lvl}** уровня!",
                        delete_after=15)
            else:
                pass
            SQL.execute(f"UPDATE users SET lvl={lvl}, xp={xp}, msg={msgR} WHERE id={msg.author.id}")
            db.commit()
...
async def top(ctx, arg=None):
    ...
    await ctx.message.delete()
    if arg == None:
        await ctx.("Вы не ввели аргумент!", delete_after=10)
    elif arg not in ["msg", "lvl"]:
        await ctx.send("введен неопознанный аргумент!", delete_after=10)
    else:
        SQL.execute(f"SELECT * FROM users ORDER BY {arg} DESC LIMIT 0, 10")
        res = SQL.fetchall()
...
async def my(ctx):
    ...
    SQL.execute(f"SELECT * FROM users WHERE id={ctx.author.id}")
    res = SQL.fetchall()[0]
    ...
    try:
        ...
        await ctx.send(f"{ctx.author.mention} Так как у тебя выключен лс, напишу тут.", embed=embed,
                       delete_after=15)
